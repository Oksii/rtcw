...
CONF_SERVERCONF=${SERVERCONF:-"comp"}
CONF_SETTINGSGIT=${SETTINGSURL:-"https://github.com/Oksii/rtcw-config-priv.git"}
...
for file in "${GAME_BASE}/rtcwpro/maps/"*.{script,spawns}; do
    [ -f "${file}" ] || continue
    rm -f "${file}"
done
